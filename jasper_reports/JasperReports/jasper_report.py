...
class JasperReport:
    def __init__(self, file_name='', path_prefix=''):
        ...
        _logger.info('-------mrissa JasperReport file_name-----%s : ',file_name)
    ...
    def standard_directory(self):
        ...
        _logger.info('-------mrissa JasperReport jasperdir-----%s : ',jasperdir)
        if jasperdir:
            if jasperdir.endswith(os.sep):
                return jasperdir
            else:
                return os.path.join(jasperdir, '')
        xx=os.path.join(os.path.abspath(os.path.dirname(__file__)),
                            '..', 'report', '')
        _logger.info('-------mrissa JasperReport extract_fields jasperdir xx-----%s : ',xx)
        return os.path.join(os.path.abspath(os.path.dirname(__file__)),
                            '..', 'report', '')

    def extract_fields(self, field_tags, ns):
        # fields and fieldNames
        _logger.info('-------mrissa JasperReport extract_fields field_tags-----%s : ',field_tags)
        _logger.info('-------mrissa JasperReport extract_fields ns-----%s : ',ns)
        fields = {}
        field_names = []
        for tag in field_tags:
            ...
            _logger.info('-------mrissa JasperReport extract_fields path1-----%s : ',path)
            # Make the path relative if it isn't already
            if path.startswith('/data/record/'):
                path = self.path_prefix + path[13:]
                _logger.info('-------mrissa JasperReport extract_fields path2-----%s : ',path)
            ...
            _logger.info('-------mrissa JasperReport extract_fields new_path-----%s : ',new_path)
            path = '/'.join(new_path)
            fields[path] = {
                'name': name,
                'type': type,
            }
            _logger.info('-------mrissa JasperReport extract_fields ields[path]-----%s : ',fields[path])
            field_names.append(name)
        
        _logger.info('-------mrissa JasperReport extract_fields fields-----%s : ',fields)
        _logger.info('-------mrissa JasperReport extract_fields field_names-----%s : ',field_names)
        return fields, field_names

    def extract_properties(self):
        # The function will read all relevant information from the jrxml file
        _logger.info('-------mrissa extract_properties ------s ------%s : ',self.report_path)
        ...
        lang_tags = doc.xpath('/jr:jasperReport/jr:queryString',
                              namespaces=nss)
        _logger.info('-------mrissa extract_properties ------s -lang_tags-----%s : ',lang_tags)
        if lang_tags:
        ...
        _logger.info('-------mrissa extract_properties ------s -relation_tags-----%s : ',relation_tags)
        ...
        for tag in dataset_tags:
            ...
            _logger.info('-------mrissa extract_properties ------s -fields-----%s : ',fields)
            _logger.info('-------mrissa extract_properties ------s -field_names-----%s : ',field_names)
            _logger.info('-------mrissa extract_properties ------s -relations-----%s : ',relations)
            _logger.info('-------mrissa extract_properties ------s -copies_field-----%s : ',copies_field)
            _logger.info('-------mrissa extract_properties ------s -copies-----%s : ',copies)
            _logger.info('-------mrissa extract_properties ------s -data_source_expression-----%s : ',data_source_expression)
            _logger.info('-------mrissa extract_properties ------s -path_prefix-----%s : ',path_prefix)
