...
class BrowseDataGenerator(AbstractDataGenerator):
    def __init__(self, report, model, env, cr, uid, ids, context):
        _logger.info('-------mrissa BrowseDataGenerator report-----%s : ',report)
        _logger.info('-------mrissa BrowseDataGenerator model-----%s : ',model)
        _logger.info('-------mrissa BrowseDataGenerator ids-----%s : ',ids)
        _logger.info('-------mrissa BrowseDataGenerator context-----%s : ',context)
        _logger.info('-------mrissa BrowseDataGenerator env-----%s : ',env)
        self.report = report
        self.model = model
        self.env = env
        self.cr = cr
    ...
    def value_in_all_languages(self, model, id, field):
        _logger.info('-------mrissa value_in_all_languages id-----%s : ',id)
        ...
        for key, value in values.items():
            ...
            _logger.info('-------mrissa value_in_all_languages result-----%s : ',result) 
        return '|'.join(result)

    def generate_ids(self, record, relations, path, current_records):
        _logger.info('-------mrissa BrowseDataGenerator generate_ids record-----%s : ',record)
        _logger.info('-------mrissa BrowseDataGenerator generate_ids path-----%s : ',path)
        _logger.info('-------mrissa BrowseDataGenerator generate_ids current_records-----%s : ',current_records)
        _logger.info('-------mrissa BrowseDataGenerator generate_ids relations-----%s : ',relations)
        
        
        unrepeated = set([field.partition('/')[0] for field in relations])
        _logger.info('-------mrissa BrowseDataGenerator generate_ids unrepeated-----%s : ',unrepeated)    
        for relation in unrepeated:

            root = relation.partition('/')[0]
            if path:
                current_path = '%s/%s' % (path, root)
                _logger.info('-------mrissa BrowseDataGenerator generate_idsA current_path-----%s : ',current_path)
            else:
                current_path = root
                _logger.info('-------mrissa BrowseDataGenerator generate_idsELSE current_path-----%s : ',current_path)
            ...
            if value:
                ...
                _logger.info('-------mrissa BrowseDataGenerator generate_ids2 current_records-----%s : ',current_records)
...
class XmlBrowseDataGenerator(BrowseDataGenerator):
    def __init__(self, report, model, env, cr, uid, ids, context):
        super(XmlBrowseDataGenerator, self).__init__(report, model, env, cr,
                                                     uid, ids, context)
        _logger.info('-------mrissa XmlBrowseDataGenerator model-----%s : ',model)
        _logger.info('-------mrissa XmlBrowseDataGenerator ids-----%s : ',ids)
        _logger.info('-------mrissa XmlBrowseDataGenerator context-----%s : ',context)
        _logger.info('-------mrissa XmlBrowseDataGenerator env-----%s : ',env)
        self.all_records = []
        self.document = None
    ...
    def generate(self, file_name):
        _logger.info('-------mrissa XmlBrowseDataGenerator  generate ids-----%s : ',file_name)
        self.all_records = []
        ...
        _logger.info('-------mrissa XmlBrowseDataGenerator self.ids-----%s : ',self.ids)
        for record in self.env[self.model].browse(self.ids):

            new_records = self.generate_ids(record, relations, '',
                                            [{'root': record}])
            copies = 1
            _logger.info('-------mrissa XmlBrowseDataGenerator new_records-----%s : ',new_records)
    ...
    def generate_xml_record(self, record, records, record_node, path, fields):
        ...
        _logger.info('-------mrissa XmlBrowseDataGenerator generate_xml_record records-----%s : ',records)
        _logger.info('-------mrissa XmlBrowseDataGenerator generate_xml_record record_node-----%s : ',record_node)
        _logger.info('-------mrissa XmlBrowseDataGenerator generate_xml_record path-----%s : ',path)
        _logger.info('-------mrissa XmlBrowseDataGenerator generate_xml_record fields-----%s : ',fields)
        ...
        for field in unrepeated:
            ...
            _logger.info('-------mrissa XmlBrowseDataGenerator generate_xml_record value_node-----%s : ',value_node)
            _logger.info('-------mrissa XmlBrowseDataGenerator generate_xml_record field_node-----%s : ',field_node)
...
class CsvBrowseDataGenerator(BrowseDataGenerator):
    ...
    # they will imply a LEFT JOIN like behaviour on the rows to be shown.
    def generate(self, file_name):
        _logger.info('-------mrissa CsvBrowseDataGenerator generate file_name-----%s : ',file_name)
        self.all_records = []
        relations = self.report.relations
        _logger.info('-------mrissa CsvBrowseDataGenerator generate relations-----%s : ',relations)
        # The following loop generates one entry to allRecords list
        # for each record that will be created. If there are any relations
        # it acts like a LEFT JOIN against the main model/table.
        reportCopies = self.report.copies or 1
        _logger.info('-------mrissa CsvBrowseDataGenerator generate reportCopies-----%s : ',reportCopies)
        sequence = 0
        copiesField = self.report.copies_field
        _logger.info('-------mrissa CsvBrowseDataGenerator generate copiesField-----%s : ',copiesField)
        for record in self.env[self.model].browse(self.ids):
            _logger.info('-------mrissa CsvBrowseDataGenerator generate record-----%s : ',record)
            _logger.info('-------mrissa CsvBrowseDataGenerator generate self.ids-----%s : ',self.ids)
            _logger.info('-------mrissa CsvBrowseDataGenerator generate relations-----%s : ',relations)
    ...
    def generateCsvRecord(self, record, records, row, path, fields, sequence,
        ...
        # Process each "root" field only once by using a set.
        unrepeated = set([field.partition('/')[0] for field in fields])
        _logger.info('-------mrissa CsvBrowseDataGenerator generateCsvRecord records-----%s : ',records)
        _logger.info('-------mrissa CsvBrowseDataGenerator generateCsvRecord row-----%s : ',row)
        _logger.info('-------mrissa CsvBrowseDataGenerator generateCsvRecord fields-----%s : ',fields)
        _logger.info('-------mrissa CsvBrowseDataGenerator generateCsvRecord sequence-----%s : ',unrepeated)
        _logger.info('-------mrissa CsvBrowseDataGenerator generateCsvRecord sequence-----%s : ',sequence)
        for field in unrepeated:
            root = field.partition('/')[0]
            _logger.info('-------mrissa CsvBrowseDataGenerator   root-----%s : ',root)
            if path:
                current_path = '%s/%s' % (path, root)
            else:
                current_path = root
            ...
            if current_path not in self.report.fields:
                continue
            _logger.info('-------mrissa     current_path-----%s : ',current_path)
            _logger.info('-------mrissa     self.report.fields-----%s : ',self.report.fields)
            # Show all translations for a field
            type  = self.report.fields[current_path]['type']
            _logger.info('-------mrissa CsvBrowseDataGenerator generateCsvRecord type-----%s : ',type)
            if type == 'java.lang.Object':
                _logger.info('-------mrissa CsvBrowseDataGenerator generateCsvRecord record._name-----%s : ',record._name)
                _logger.info('-------mrissa CsvBrowseDataGenerator generateCsvRecord record.id-----%s : ',record.id)
                _logger.info('-------mrissa CsvBrowseDataGenerator generateCsvRecord root-----%s : ',root)
                value = self.value_in_all_languages(record._name, record.id,
                                                    root)
            _logger.info('-------mrissa CsvBrowseDataGenerator generateCsvRecord value-----%s : ',value)   
