...
def searchargs(params, kwargs=None, context=None):
    ...
    if not params:
        return domain
    _logger.info("_____mrissa searchargs ok-----params =%s",params)
    _logger.info("_____mrissa searchargs ok-----params =%s",params[0])
    _logger.info("_____mrissa searchargs ok-----enumerate(domain) =%s",enumerate(domain))
    domain = params[0]
    if not isinstance(domain, list):
        return params
    for (idx, term) in enumerate(domain):
        _logger.info("_____mrissa searchargs ok-----enumerate(domain) =(%s,%)",idx, term)
...
def authenticate(token):
    try:
        a = 4 - len(token) % 4
        if a != 0:
            _logger.info("_____mrissa pdp ok-----a =%s",a)
            token += '==' if a == 2 else '='
            _logger.info("_____mrissa pdp ok-----token =%s",token)
            data_bytes = str(token).encode("utf-8") 
        SERVER,db,login,uid,ts = base64.urlsafe_b64decode(data_bytes).decode('ascii').split(',')
        _logger.info("_____mrissa pdp ok-----SERVER,db,login,uid,ts =%s<>%s<>%s<>%s<>%s<>",SERVER,db,login,uid,ts)
...
class VneuronRestfulApi(http.Controller):
    @http.route([
        '/api/v1.0/get_token',
        ], type='http', auth="none", csrf=False, methods=['POST','GET'])
    def get_token(self,serv='', a=None, s=None, db='sige_prod_2', sid=None, success=True, message='',**kw):
        """ service, app(user/login),secret(password) """
        #pdb.set_trace()
        _logger.info("_____mrissa pdp ok-----a =%s",a)
        _logger.info("_____mrissa pdp ok-----s =%s",s)
        _logger.info("_____mrissa pdp ok-----datavase =%s",db)
        serv=request.httprequest.host_url
        
        try:
            uid = http.request.session.authenticate(db, a, s)
            _logger.info("_____mrissa pdp ok-----uid =%s",uid)
        except Exception as e:
        ...
        if not uid:
            ...
            return json_response(rp)
        wwwwc= ','.join([serv,db,a,str(uid),str(int(time.time()))])
        data_bytes = wwwwc.encode("utf-8")
        _logger.info("_____mrissa pdp ok-----wwwwc =%s",wwwwc)
        e = base64.urlsafe_b64encode(data_bytes).decode('ascii')
    ...
    def read_objects(self, model=None, ids=None, **kw):
        ...
        _logger.info("_____mrissa pdp ok-----env =%s",env)
        if not env:
            return no_token()
        ...
        _logger.info("_____mrissa pdp ok-----domain =%s",domain)
        ...
        try:
            ...
            _logger.info("_____mrissa pdp ok-----model_fields =%s",model_fields)
            for r in result:
                for f in r.keys():
    ...
    def unlink_objects(self, model=None, ids=None, success=True, message='', **kw):
        token = kw.pop('token')
        env = authenticate(token)
        _logger.info("_____mrissa pdp ok-----ids =%s",ids)
        if not env:
            return no_token()
        ids = map(int,ids.split(','))
        try:
