...
from odoo.addons.vneuron_workflow_odoo.workflow.helpers import Session
from odoo.addons.vneuron_workflow_odoo.workflow.helpers import Record
from odoo.addons.vneuron_workflow_odoo.workflow.helpers import WorkflowActivity
...
class WorkflowItem(object):
    ...
    def process(self, signal=None, force_running=False, stack=None):
        ...
        logger.info('____________ process WorkflowItem_ activity____%s',activity)
        triggers = False
        ...
        if force_running or self.workitem['state'] == 'complete':
            logger.info('____________ process WorkflowItem_ force_running____%s',activity)
        ...
        if split_mode in ('XOR', 'OR'):
            ...
            for transition in alltrans:
                ...
                if not cr.fetchone()[0]:
                    transitions.append((transition['id'], self.workitem['inst_id']))
        logger.info('____________ _split_test WorkflowItem__transitions___%s',transitions)
    ...
    def _join_test(self, trans_id, inst_id, stack):
        ...
        activity = cr.dictfetchone()
        logger.info('____________ _join_test WorkflowItem___join_test___%s',activity)
    ...
    def wkf_expr_eval_expr(self, lines):
        ...
        logger.info('____________ wkf_expr_eval_expr WorkflowItem__result___%s',result)
    ...
    def wkf_expr_check(self, transition, signal):
        ...
        if self.session.uid != odoo.SUPERUSER_ID and transition['group_id']:
            ...
            if transition['group_id'] not in user_groups.ids:
                return False
        logger.info('____________ wkf_expr_check WorkflowItem__transition___%s',transition)
